...
## 9. Производительность и что из неё следует

Результаты бенчмарка на 6000 акторов и 30 миллионов сообщений дают ориентиры для проектирования прикладных акторов. Вот ключевые выводы.

### Цифры

- **2,3 миллиона сообщений в секунду** — при тысячах активных акторов
- **0,43 микросекунды на сообщение** — включая доставку и обработку
- **52 миллисекунды на создание 6000 акторов** — создание практически бесплатно
- **36,5 МБ памяти на 6000 акторов** — около 6 КБ на актор с очередью в 256 сообщений
- **30 потоков при пиковой нагрузке** — пул потоков масштабируется под нагрузку автоматически
- **0 потерянных сообщений** — `DropWrite` при переполнении очереди ни разу не сработал

### Что это значит для ваших акторов

**Акторы дёшевы.** Не бойтесь создавать сотни и тысячи акторов. Каждый сенсор, каждое соединение, каждая сессия пользователя — всё может быть отдельным актором. Архитектурного ограничения на количество нет.

**Сообщения быстрее любых внешних операций.** 0,43 мкс на сообщение — это примерно в 10 000 раз быстрее чтения с диска и в 100 000 раз быстрее сетевого вызова. Вся координация внутри акторной системы пренебрежимо дешёва по сравнению с любым вводом-выводом.

**Память не проблема.** 6 КБ на актор — меньше, чем размер HTTP-запроса. Даже на скромном сервере с 1 ГБ свободной памяти можно держать десятки тысяч акторов.

**Потоки масштабируются автоматически.** Не нужно настраивать пулы или лимиты. Акторы не привязаны к потокам — один поток обслуживает много акторов, переключаясь между ними через `await`.

### На что тратится память

Из 2760 МБ выделенной памяти большая часть — это сами сообщения (30 миллионов писем). Каждое письмо — это объект на куче. Если в вашей системе циркулируют миллионы сообщений, размер письма имеет значение. Используйте структуры данных внутри письма экономно, избегайте вложенных коллекций и длинных строк.

### Ограничения системы

**Очередь ограничена.** По умолчанию 256 сообщений на актор. Если актор обрабатывает медленно (например, ждёт внешний HTTP), а отправители шлют быстро — очередь заполнится, и новые сообщения будут молча отброшены. Для быстрых внутренних операций (как в бенчмарке — `Ping`/`Pong` без ввода-вывода) переполнение не наступает. Но если актор ходит в базу данных или внешний сервис — увеличьте `QueueCapacity` при создании или поставьте Breaker/Trottle перед ним.

**Обработка строго последовательна.** Один актор — одно сообщение в момент времени. Если актор выполняет длинную операцию (например, 5 секунд ждёт HTTP-ответ), его очередь будет стоять. Выносите длинные ожидания в отдельный актор или используйте таймауты для освобождения очереди.
